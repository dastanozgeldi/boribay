...
from boribay.utils import Cog, Manip, make_image, make_image_url, polaroid_filter
...
class Images(Cog):
    ...
    async def dagpi_image(self, url, fn=None):
        r = await self.bot.session.get(
            f'{self.bot.config.api.dagpi[0]}{url}',
            headers={'Authorization': self.bot.config.api.dagpi[1]}
        )
        return discord.File(BytesIO(await r.read()), fn or 'dagpi.png')

    async def alex_image(self, url, fn=None):
        r = await self.bot.session.get(
            f'{self.bot.config.api.alex[0]}{url}',
            headers={'Authorization': self.bot.config.api.alex[1]}
        )

...
from boribay.utils import Cog, Trivia
...
class Economics(Cog):
    ...
    icon = '💵'
    name = 'Economics'
    bad_words = open('./boribay/data/textfiles/badwords.txt', 'r').readlines()[1].split()

    async def cog_check(self, ctx):
        return await commands.guild_only().predicate(ctx)

    async def double(self, ctx, choice: str, amount: int, reducer, adder):
    ...
    async def question(self, ctx, difficulty: str):
        ...
        r = await ctx.bot.session.get(f'{ctx.config.api.trivia}&difficulty={difficulty}')
        js = await r.json()
        js = js['results'][0]
    ...
    async def answer(self, ctx, q):
        ...
        entries = [q['correct_answer']] + q['incorrect_answers']
        answer = await Trivia(
            random.sample(entries, len(entries)), q['question']
        ).pagination(ctx)
    ...
    @commands.command()
    async def pay(self, ctx, amount: int, member: commands.MemberConverter):
    ...
    async def attack(self, ctx, member: commands.MemberConverter):
